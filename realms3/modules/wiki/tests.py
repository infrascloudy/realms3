...
from realms3.lib.test import BaseTest


class WikiBaseTest(BaseTest):
    def update_page(self, name, message=None, content=None):
        return self.client.post(
            url_for("wiki.page_write", name=name),
            data=dict(
                message=message, content=content, csrf_token=self.client.csrf_token
            ),
        )
...
class WikiTest(WikiBaseTest):
    ...
    def test_revert(self):
        ...
        self.client.post(
            url_for("wiki.revert"),
            data=dict(
                name="test", commit=data["sha"], csrf_token=self.client.csrf_token
            ),
        )
    ...
    def test_anon(self):
        ...
        self.assert_403(
            self.update_page("test", message="test message", content="testing_again")
        )
        self.assert_403(
            self.client.post(
                url_for("wiki.revert"),
